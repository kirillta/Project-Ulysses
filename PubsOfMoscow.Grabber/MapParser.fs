...
let getPubListAsString (opts: ParseOptions) = 
...
let rec getTypeTags (acc: string list) count (matches: MatchCollection) =
    match count with 
    | 0 -> acc
    | _ -> 
...
let rec getPubTypes acc (source: string) (tags: string list) = 
...
let getCoordinates source = 
    ...
    | true ->
        ...
        let long = Decimal.Parse(m.Groups.[1].Value)
        let lat = Decimal.Parse(m.Groups.[2].Value)
        (long, lat)
